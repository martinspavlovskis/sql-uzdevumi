...
const insertMovies = (movies: Movie[]) => {
  return (
    ...
    movies.map(movie => `('${escape(movie.imdbId)}', ${movie.popularity}, ${movie.budget}, ${movie.budgetAdjusted}, ${movie.revenue}, ${movie.revenueAdjusted}, '${escape(movie.originalTitle)}', '${escape(movie.homepage? movie.homepage : "null")}', '${escape(movie.tagline? movie.tagline : "null")}', '${escape(movie.overview)}', ${movie.runtime}, '${movie.releaseDate}')`).join(","));
};
